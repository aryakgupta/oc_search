...
/*
$solr = new Apache_Solr_Service('192.168.92.67', 8983, '/solr'); //or explicitly new Apache_Solr_Service('192.168.92.67', 8983, '/solr')
...
if ($solr->ping())
{
 		...
 		$document->name = 'Some Title';
 		$document->features = 'Some content for this wonderful document. Blah blah blah.';
 		...
 		$result = $solr->search('features:blah'); //, 0, 10, array('sort' => 'timestamp desc')
...
class Apache_Solr_Service
{
	...
	const SPELLSEARCH_SERVLET = 'spell';
	 ...
	 * @param string $path
	 */
	public function __construct($host = '192.168.61.32', $port = 8080, $path = '/zig_main_solr/')
	{
		$this->setHost($host);
		$this->setPort($port);
		$this->setPath($path);
	...
	private function _initUrls()
	{
		...
		$this->_spellsearchUrl = $this->_constructUrl(self::SPELLSEARCH_SERVLET);
	...
	public function ping($timeout = 5)
	{
		...
		if ($fp)
		{
			...
			if (array_key_exists("timeout", $metaData))
			{
				if($metaData['timeout'])
				{
					fclose($fp);
			...
			if (array_key_exists("timeout", $metaData)) //$metaData['timeout'])
			{
				if($metaData['timeout'])
				{
					fclose($fp);
...
//echo implode($this->_queryStringDelimiter, $escapedParams)."<br>";
		return $this->_sendRawGet($this->_searchUrl . $this->_queryDelimiter . implode($this->_queryStringDelimiter, $escapedParams));
	}
	
	public function spellsearch($query, $offset = 0, $limit = 10, $params = array())
	{
		if (!is_array($params))
		{
			$params = array();
...
//echo implode($this->_queryStringDelimiter, $escapedParams)."<br>";
		return $this->_sendRawGet($this->_spellsearchUrl . $this->_queryDelimiter . implode($this->_queryStringDelimiter, $escapedParams));
