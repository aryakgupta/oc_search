...
WHERE
    p.status =0";
  echo  $qqqqqq.=" ORDER BY p.product_id DESC LIMIT $start, $end"; 
...
$time_start = microtime(true);
   ...
   while($rs=mysql_fetch_assoc($qqq1)){
      $master[]=$rs['product_id'];    
    $i++;
    }   
		include_once(dirname(__FILE__)."/solr/classes/BharatProductIndexDelete.class.php");
        $obj=new BharatProductIndexDelete();
        if((is_array($master) || is_object($master)) && !empty($master)){
			$productId=implode("~",$master);
            $params=array();
